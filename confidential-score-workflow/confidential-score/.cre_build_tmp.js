...
var TEST_API_URL = "https://postman-echo.com/post";
var HEX = /^[0-9a-fA-F]+$/;
function bytesToHex(bytes) {
  return Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join("");
}
function validateEncryptedOutput(out) {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  const apiKeySecret = runtime2.config.apiKeySecretName;
  const aesKeySecret = runtime2.config.aesEncryptionKeySecretName;
  ...
  const bodyBytes = response.body instanceof Uint8Array ? response.body : new Uint8Array(Buffer.from(response.body || "", "base64"));
...
var initWorkflow = (config, _secretsProvider) => {
  ...
  const authorizedKeys = config.authorizedPublicKey ? [{ type: "KEY_TYPE_ECDSA_EVM", publicKey: config.authorizedPublicKey }] : [];
  return [
